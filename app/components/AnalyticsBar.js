...
const AnalyticsBar = ({ prediction, probability, bulkResults }) => {
  ...
  const uploadToBlockchain = async (result) => {
    ...
    await transactionStorage.methods
      .storeTransaction(
        result.prediction === 0 ? "No Money Laundering" : "Laundering Detected",
        result.probability && result.probability.length > 1
          ? result.probability[1].toFixed(3)
          : "N/A",
        result.probability && result.probability.length > 0
          ? result.probability[0].toFixed(3)
          : "N/A"
      )
        ...
        <tbody>
          {filteredResults.map((result) => (
            <tr key={result.recordId}>
              ...
              <td className="text-center p-2 border-b-2">
                <button
                  onClick={() => uploadToBlockchain(result)}
                  type="button"
                  class="text-gray-900 transition duration-300 hover:scale-105 bg-gray-100 hover:bg-blue-300 focus:ring-4 focus:outline-none focus:ring-gray-100 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-gray-500 me-2 "
                >
                  <svg
                    class="w-4 h-4 me-2 -ms-1 text-[#919ef0]"
                    aria-hidden="true"
                    focusable="false"
                    data-prefix="fab"
                    data-icon="ethereum"
      ...
      datasets: [
        {
          data: [
            filteredResults.reduce(
              (sum, result) => sum + (result.prediction === 1 ? 1 : 0),
              0
            ),
            filteredResults.reduce(
              (sum, result) => sum + (result.prediction === 0 ? 1 : 0),
              0
            ),
          ],
  ...
  if (!probability || probability.length < 2) {
    return (
      <p className="text-3xl outline outline-slate-600 rounded-lg p-3 m-3 mt-5 w-101 min-h-screen">
  ...
  return (
    <div className="outline outline-slate-600 rounded-lg p-3 m-3 mt-5 w-101 min-h-screen">
      {prediction !== null && probability !== null ? (
        <div>
          <h3 className="text-5xl mt-3 mb-3">
            Prediction:{" "}
            {prediction === 0 ? "No Money Laundering" : "Laundering Detected"}
          </h3>
          <p className="text-3xl mb-3">
            Money laundering: {probability[1].toFixed(3)}
          </p>
          <p className="text-3xl mb-3">
            Not money laundering: {probability[0].toFixed(3)}
          </p>
          <div className="flex justify-start mt-4">
            
            <button
              type="button"
              class="text-gray-900 bg-gray-100 hover:bg-blue-300 hover:text-white hover:scale-110 transition duration-300 focus:ring-4 focus:outline-none focus:ring-gray-100 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-gray-500 me-2 mb-2"
              onClick={() => uploadToBlockchain({ prediction, probability })}
            >
              <svg
                class="w-4 h-4 me-2 -ms-1 text-[#919ef0]"
                aria-hidden="true"
